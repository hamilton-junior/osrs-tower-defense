...
**Every interface respects the UI resize.** `--ui-scale` is the game's *only* zoom, so anything the player reads must grow with it — not just DOM panels. **Canvas-drawn interface overlays count**: the tower level/XP plate and anything like it multiply their logic-px sizes by `GameEngine.uiScale` (set from GameRoot's `--ui-scale` effect via `setUiScale`), instead of being a fixed number of logic pixels. Exempt only where it was decided otherwise on purpose: the fixed 1440x640 board and the bottom bar's constant `4.3em` height. Game *entities* — enemies, towers, projectiles, range squares — are world objects, not interface, and stay in logic px.
...
**The tutorial mirrors the UI.** `LEARN_STEPS` (contextual tips, one at a time, anchored to a `data-tut` target) and `TLDR` (the How-to-Play tabs) both live in `components/game/tutorial.tsx` and must describe the same real interface at different depth. Change one, change the other. Existing anchors: `map`, `hud`, `sidebar` (the whole bar), `stones` (just the interface stones), `controls`, `uiscale`, `dock`, `startwave`, `prayers`, `waveevent`, `slayer`, `essence`, `help`. **An anchor must hug what the tip is about** — the ring is drawn around the element's box, so tagging a `flex-1` group rings its empty stretch too; wrap the real subject in a tight inner element and tag that. There is no `ge` anchor (the Grand Exchange menu was removed) and no debug stone (the console opens on `Ctrl+'`).

**And both are short on purpose.** They were paragraphs once and players read neither. A tip is **one sentence**; a How-to-Play line is **one idea, about a dozen words**, led by an OSRS sprite instead of an explanation, and each of the five tabs fits its well without scrolling (`min-h` on the well is sized to the tallest tab so switching doesn't resize the window). Detail the interface already gives on hover — gear ladders, copy/paste, difficulty tiers, achievements — is deliberately **not** repeated here. Adding a system means adding a *line*; if it needs a paragraph, it belongs in the panel that owns it.
