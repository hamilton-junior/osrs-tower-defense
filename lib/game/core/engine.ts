...
export class GameEngine {
  ...
  /** One carried or stored stack as the interface draws it. The engine is the only
  ...
  private snapshot(): UIState {
    return {
      ...
      bagStacks: this.items.bag.map(s => this.uiStack(s)),
  ...
  private preloadImages() {
    const urls: Record<string, string> = {
      ...
      // The fishing spot's own ripple, one eight-frame strip of its stand animation
      // per state: the busy Tempoross spot while there are fish in the pool, and the
      // ordinary spot — the same mesh at the fainter alpha the cache gives it — once
      // there are not.
   ...
   *  slot free it goes to the looting bag instead, so a full inventory costs the
   *  herb a trip rather than losing it. */
  harvestPatch(patchId: string) {
    ...
    regionTally(this.caStats, this.biome.id).herbs += 1;
    this.sound.play('farm_harvest');
  ...
  // ---------------------------------------------------------------- inventory
  // Twenty-eight slots that never stack, and the looting bag behind them. See
  ...
  /** Pull one back out of the bag. A full inventory refuses it outright, and that
   *  one does need saying: the slot looked live and nothing happened. */
  takeFromBag(kind: StackKind, id: string) {
    if (toInv(this.items, kind, id, 1) < 1) {
  ...
  brewPotion(potionId: PotionId) {
    ...
    regionTally(this.caStats, this.biome.id).potions += 1;
    const gain = gainHerbloreXp(this.herbloreLevel, this.herbloreXp, def.xp);
    this.herbloreLevel = gain.level;
    this.herbloreXp = gain.xp;
    this.sound.play('farm_harvest');
    this.notify(where === 'bag' ? `${def.name} brewed, to the loot bag` : `${def.name} brewed`, def.icon);
    if (gain.levels > 0) {
  ...
  private landCatch() {
    ...
    if (fish) {
      ...
      regionTally(this.caStats, this.biome.id).fish += 1;
  ...
  loadRun(save: RunSave) {
    ...
    // resumed half-shaped.
    this.caStats = { ...emptyRunStats(this.gameMode, this.difficultyTier), ...save.caStats };
    visitRegion(this.caStats, this.biome.id);
  ...
  restart() {
    ...
    // The map may already have been regenerated by the time the facts are cleared,
    // so the region the run actually starts in is stamped back on afterwards.
    visitRegion(this.caStats, this.biome.id);
    this.waveTotal = 0;
    this.bossWave = false;
    this.activeEvent = null;
