...
**Watermark (2026-09-04):** Suggestions ≤ **39**, Bug Reports ≤ **25**.
Re-queried 2026-09-04 (the MCP server was unreachable for days before that): three new
rows — Suggestions 39, Bug Reports 24 and 25 — triaged into the tables below.
The previous pass, 2026-08-25, found nothing past Suggestions 38 / Bug Reports 23. That pass also synced
...
| 24 | UI zoom no longer goes past 100% (was 70–120%) | shipped — `626c9ae` (2026-08-28), six hours after the report. Not a cap that was lowered: the ceiling is *measured* from the bar's own natural width (the fix for #15), and the measured value was then floored onto the 5% step grid. A 1920-wide row that holds 107% was handed 100% with "+" greyed out — indistinguishable from a hard cap, and reported as one. The ceiling is kept at its measured value now and the last press is allowed to be a short step, so the leftover room is reachable |
| 25 | Dawn back-tracks over a shifted road leg | shipped — `8502609` (2026-09-04). Dawn is the only enemy with a `laneOffset` (she flies one lane clear of Dusk). `moveEnemies` (`core/sim/waves.ts`) aims her at a point *perpendicular* to the current segment and advances `pathIndex` only once she is within 4px of **that offset point**. On a straight leg the offset point sits neatly ahead; on the tight corner pair a road shift (**P2a**) carves out, the next segment's offset point can lie *behind* her, so she overshoots, turns round to touch it, and crosses again — exactly what the report describes. The board is not confused and the path is not broken: only the arrival test is. The leg now ends when she draws *level* with the aim point — the plane through it, perpendicular to the segment — instead of when she touches it, so a corner sharper than the lane is wide makes her cut it rather than fly back for it. The whole rule moved out of the sim into `laneLeg` in `systems/geometry.ts` with five tests, one of them the shifted leg from the report. An enemy with no lane offset keeps the plain distance test and is untouched, so nothing but Dawn changes |
...
| 26 | Game ideas (11-item list) | **mostly shipped** — the road stopped reading as a car lane (`8fd8f66`), and since then: the tower-spam curb (+15% per same-type tower, `5d24d7d`), tower XP, a win condition (**A1**), monster drops, traps (**P2b**) and generated maps (**M7**) all landed. What is left is what the roster and the focus rule out — branching upgrades and melee-spawning towers (the roster is CLOSED), hand-drawn maps (the board is generated per run), a campaign and player resources (**M4**-shaped, descartado) |
...
| 30 | Clue Scrolls | queued **[later]** — accepted as a real direction, but for a "not very near future" (user, 2026-08-21). Net-new content: draggable clue map, dig spots, 3rd-age gear. The golden tower this originally hung its reward on is **discarded** (user, 2026-09-21) — the scrolls stand without it. Now has a designed sibling — **F4** below — so treat them as one piece of work |
...
| 35 | Construction (Mazing) mode | **shipped** — both parts the user cared about landed as **P2** (2026-08-24): **P2a** lets the player buy a bend in any interior road leg, and **P2b** lays Hunter traps *on* the road. The full separate mode was never the ask and stays off the list; free-form mazing is deliberately barred (the bend price climbs 1.55x per bend) |
...
| 39 | Streamer NPCs as a between-waves easter egg | **needs a call (user)** — Will, 2026-08-26, via the B0aty channel. Asks for content creators (B0aty, Odablock, EV Scape, Settled…) to walk on as NPCs among the between-wave diversions, and says himself it "is really not necessary at all". Two things block it rather than the idea being bad: the **walkby mood of the D&D frame (item 21) is exactly the slot it wants**, so the plumbing is free — but every asset in this game is baked from the local OSRS cache, and a streamer is not an NPC in that cache. The nearest honest bake is a player-model avatar dressed in their known outfit, which is a likeness of a real person shipped without their say-so. So this is the user's call twice over: whether to do it at all, and if so which model stands in |
